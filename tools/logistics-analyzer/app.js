...
範例格式：
1、「一般商品」問題佔比52%：
①訂購BX-48 隨機強化組（商代1422103、4904810096092），因庫存不足或需確認是否可訂購，導致訂單無法順利成立或出貨（60筆）。訂購一番賞落地陳列架（商代20160560），店鋪申請補發，客服已代訂購（217筆）。
②退貨單0183581583單據未回提醒單、退貨單0247764346商品與贈品被誤收，要求確認退貨單及商品後續是否入帳（25筆）。

2、「貨單問題」問題佔比8%：
①店鋪反應進貨單或退貨單資料錯誤，如寶可夢卡牌進貨數量KEY錯、LAIMO購物袋退貨數量KEY錯，要求協助修正或確認帳務（21筆）。
②退貨單0223664493有被反修，要求協助確認入帳（2筆）。

規則：
- 商品名稱後加（商代XXXXXX）
- 每句結尾加（XX筆），筆數用統計資料中的數字
- 不用「例如」，直接描述實際案件內容
- 只輸出2大類，每類2小點（①②）
- 直接輸出文字，不需標題或結語
